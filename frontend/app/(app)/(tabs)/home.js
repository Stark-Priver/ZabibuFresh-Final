...
import { getProducts, getConversations } from '../../services/supabase';
  ...
  const fetchDashboardStats = async () => {
    ...
    try {
      ...
      if (profile.role === 'seller') {
        // Fetch seller stats
        const { data: products, error: productsError } = await getProducts(profile.id);
        
        if (productsError) throw productsError;

        const { data: conversations, error: messagesError } = await getConversations(profile.id);
        
        if (messagesError) throw messagesError;

        // Count messages received by this seller
        const receivedMessages = conversations?.filter(msg => msg.receiver_id === profile.id) || [];
        ...
        // Count messages sent by this buyer
        const sentMessages = conversations?.filter(msg => msg.sender_id === profile.id) || [];
