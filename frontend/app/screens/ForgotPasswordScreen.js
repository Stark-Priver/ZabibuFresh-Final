...
const ForgotPasswordScreen = () => {
  ...
  const handlePasswordReset = async () => {
    ...
    if (error) {
      Alert.alert('Error Sending Recovery Code', error.message);
      setMessage(`Error: ${error.message}`);
    } else {
      Alert.alert('Recovery Code Sent', 'Please check your phone for a code to reset your password.');
      setMessage('If an account exists for this phone number, a recovery code has been sent.');
    }
    setLoading(false);
  };
